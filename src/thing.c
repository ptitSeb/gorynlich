...
int server_things_total;
int client_things_total;
int server_monst_things_total;
int client_monst_things_total;
                         ...
                         thing_statsp stats)
{
    ...
    *next = id + 1;
    ...
    /*
     * Quick hack to make stuff glow
     */
    if (tp_to_id(tp) == THING_DEATH) {
        thing_wield_shield(t, tp_find("death_cloud"));
    }

    if (tp_to_id(tp) == THING_JESUS) {
        thing_wield_shield(t, tp_find("jesus_cloud"));
        MSG_SERVER_SHOUT_AT_ALL_PLAYERS(SOUND_GLOBAL, x, y, "jesus");
    }

    if ((tp_to_id(tp) == THING_RING1) ||
        (tp_to_id(tp) == THING_POTION_JESUS) ||
        (tp_to_id(tp) == THING_RING2) ||
        (tp_to_id(tp) == THING_RING3)) {
        thing_wield_shield(t, tp_find("ring_cloud"));
    }
...
void thing_destroy (thingp t, const char *why)
{
    ...
    if (t->on_server) {
        if (thing_is_jesus(t)) {
            level_set_is_jesus_summoned(server_level, false);
        }

        if (thing_is_death(t)) {
            level_set_is_death_summoned(server_level, false);
        }
    }
...
static void thing_dead_ (thingp t, char *reason)
{
    ...
    if (thing_is_player(t)) {
         ...
         *
        MSG_SERVER_SHOUT_AT(t, CRITICAL, "Killed by %s", reason);
         */
        aplayer *p = t->player;
        if (p) {
    ...
    /*
     * thing_dying() covers players with the bell toll
     */
    if (!thing_is_player(t)) {
        const char *sound = tp_sound_on_death(t->tp);
        if (sound) {
            MSG_SERVER_SHOUT_AT_ALL_PLAYERS(SOUND, 
                                            t->x, t->y, 
                                            "%s", sound);
        }
    }
...
void thing_dead (thingp t, thingp killer, const char *reason, ...)
{
    ...
    if (t->on_server) {
        /*
         * Check for death defying special abilities
         */
        {
            tpp tp = thing_arm_left(t);
            if (tp && tp_is_life_saving(tp)) {
                thing_used(t, tp);
                return;
            }

            tp = thing_arm_right(t);
            if (tp && tp_is_life_saving(tp)) {
                thing_used(t, tp);
                return;
            }
    ...
    if (t->on_server) {
        ...
        if (thing_is_joinable(t) ||
            thing_is_player(t)) {
            level_set_needs_updating(server_level, true);
        }
...
#endif
                        MSG_SERVER_SHOUT_AT(OVER_THING,
                                            t,
                                            0, 0,
                                            "%%%%font=%s$%%%%fg=%s$+%d XP",
                                            "large", "gold",
...
void thing_game_over (thingp t)
{
    thing_dead(t, 0, "quest completed");
}
...
void thing_dying (thingp t, thingp killer, const char *reason, ...)
{
    ...
    if (t->on_server) {
         ...
         */
        if (thing_is_player(t)) {
            THING_LOG(t, "play player death bell toll");
        ...
        const char *sound = tp_sound_on_death(t->tp);
        if (sound) {
            MSG_SERVER_SHOUT_AT_ALL_PLAYERS(SOUND_GLOBAL, 
                                            t->x, t->y, 
                                            "%s", sound);
        }
    } else {
...
CON("%s hit success on %s hitter %s",
    ...
    if (thing_is_lava(hitter)) {
        thing_server_effect(t, THING_STATE_EFFECT_IS_FLAMES);
    }

    if (thing_is_acid(hitter)) {
        thing_server_effect(t, THING_STATE_EFFECT_IS_BUBBLES);
    }
    ...
    if (thing_is_dead(t)) {
        ...
        if (orig_damage > 0) {
            ...
            MSG_SERVER_SHOUT_AT(OVER_THING, t,
                                t->x, t->y,
                                "%%%%font=%s$%%%%fg=%s$-%d HP",
...
int thing_hit (thingp t, thingp hitter, uint32_t damage)
{
    ...
    if (hitter) {
         ...
         * Jesus is nice
         */
        if (thing_is_jesus(hitter) && thing_is_player(t)) {
            return (false);
        }
...
void things_level_destroyed (levelp level, uint8_t keep_players)
{
    ...
    if (level == server_level) {
        {
            if (keep_players) {
                LEVEL_LOG(level, "destroy things, keep players");
            } else {
                LEVEL_LOG(level, "destroy things and players");
            }
        ...
        if (keep_players) {
            LEVEL_LOG(level, "destroy boring things");
        }
        ...
        if (keep_players) {
            LEVEL_LOG(level, "check active things are gone");
        }
...
void socket_server_tx_map_update (gsocketp p, tree_rootp tree, const char *type)
{
    ...
    *data++ = MSG_SERVER_MAP_UPDATE;
    ...
    TREE_WALK_UNSAFE(tree, t) {
         ...
         */
        uint16_t template_id = tp_to_id(tp);
...
//CON("T id       0x%04x", id);
        ...
        if (state & (1 << THING_STATE_BIT_SHIFT_ID_TEMPLATE_PRESENT)) {
            SDLNet_Write16(template_id, data);
            data += sizeof(uint16_t);
//CON("  template 0x%04x", template_id);
        }
...
//CON("R id       0x%04x", id);
        ...
        if (state & (1 << THING_STATE_BIT_SHIFT_ID_TEMPLATE_PRESENT)) {
             ...
             */
            template_id = SDLNet_Read16(data);
            data += sizeof(uint16_t);
//CON("  template 0x%04x", template_id);
        } else {
            template_id = -1;
...
//CON("rx id %d create thing",id);
            if (!need_fixup && tp_is_joinable(tp)) {
                need_fixup = true;
            }
        } else {
            if (template_id != (uint16_t)-1) {
...
if (t->is_jumping) {
    ...
    CON("%s move to %f,%f",thing_logname(t),x,y);
                    ...
                    if (!need_fixup && thing_is_joinable(t)) {
                        need_fixup = true;
                    }
                }
            } else {
