...
void shop_enter (thingp t, thingp floor)
{
    ...
    if (!shopkeeper) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "No shopkeeper to be seen");
        return;
    }
    ...
    if (thing_is_dead(shopkeeper)) {
         ...
         */
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "A bad thing happened here...");
        return;
         ...
         */
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$%s", "Get out of my shop!");
        ...
        MSG_SERVER_SHOUT_AT(SOUND, t, t->x, t->y, "shotgun_reload");
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=green$%s", messages[myrand() % ARRAY_SIZE(messages)]);
    }
...
void shop_leave (thingp t, thingp floor)
{
    ...
    if (!shopkeeper) {
        ...
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "Odd... No one around to pay");
        return;
    }
    ...
    if (t->money_owed < 0) {
         ...
         */
        if (thing_is_dead(shopkeeper)) {
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "No one lives to pay you");
        } else if (thing_is_angry(shopkeeper)) {
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$%s", "I refuse to pay a thief!");
        } else {
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=green$%s", "Good doing business with you");
...
void shop_collect_message (thingp t, thingp item)
{
    ...
    if (!shopkeeper) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "Odd... no one owns this. Free item...");
        return;
    }
    ...
    if (t->timestamp_last_shop_enter &&
        ...
        if (thing_is_dead(shopkeeper)) {
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "No one to pay");
             ...
             */
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$More thievery!");
        } else {
            MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%s", messages[myrand() % ARRAY_SIZE(messages)]);

            MSG_SERVER_SHOUT_AT(WARNING, t, 0, 0, "%%%%fg=red$Press P to pay");
        }
    }
...
void shop_deposit_message (thingp t, thingp item)
{
    ...
    thingp shopkeeper = thing_server_find(t->in_shop_owned_by_thing_id);
    if (!shopkeeper) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "No one seems to be running the shop");
        return;
    }

    if (thing_is_dead(shopkeeper)) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "No one to accept these goods");
        return;
    } else if (thing_is_angry(shopkeeper)) {
        MSG_SERVER_SHOUT_AT(WARNING, t, 0, 0, "%%%%fg=red$I'm not taking your stolen goods!");
        return;
    }

    if (!tp_get_cost(item->tp)) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "I'm not taking that junk");
        return;
    }
...
void shop_pay_for_items (thingp t)
{
    ...
    thingp shopkeeper = thing_server_find(t->in_shop_owned_by_thing_id);
    if (!shopkeeper) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "There is no one to pay");
        return;
    }

    if (thing_is_dead(shopkeeper)) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "He cannot take payment now...");
        return;
    }

    if (thing_is_angry(shopkeeper)) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$I'll never take your money!");
        return;
    }
    ...
    MSG_SERVER_SHOUT_AT(SOUND, t, t->x, t->y, "payment");
}
...
void shop_steal_message (thingp t)
{
    ...
    thingp shopkeeper = thing_server_find(t->in_shop_owned_by_thing_id);
    if (!shopkeeper) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$%s", "You steal from children?");
        return;
    }

    if (thing_is_dead(shopkeeper)) {
        MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=red$%s", "You steal from the dead...");
        return;
    }
...
void shop_whodunnit_break_message (thingp t, thingp shopkeeper)
{
    ...
    MSG_SERVER_SHOUT_AT(POPUP, t, 0, 0, "%%%%fg=orange$%s", messages[myrand() % ARRAY_SIZE(messages)]);
}
